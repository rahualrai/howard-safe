...
const MOCK_DINING: Service[] = [
  { name: "Blackburn Dining", isOpen: true, hours: "7:00 AM – 8:00 PM" },
  { name: "Bethune Annex", isOpen: false, hours: "Closed • Opens 11:00 AM" },
];
...
export function DiningServices() {
  const { data: servicesData, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await (supabase
        ...
        .select("name,category,is_open,hours")) as unknown as { data: { name: string; category: "dining" | "service"; is_open: boolean; hours: string | null }[] | null, error: any };
  ...
  const dining = (servicesData && servicesData.length > 0)
    ? servicesData.filter(s => s.category === "dining").map<Service>(s => ({ name: s.name, isOpen: s.is_open, hours: s.hours ?? "" }))
    : MOCK_DINING;
  const otherServices = (servicesData && servicesData.length > 0)
    ? servicesData.filter(s => s.category === "service").map<Service>(s => ({ name: s.name, isOpen: s.is_open, hours: s.hours ?? "" }))
    : MOCK_SERVICES;
  ...
  return (
    <Card className="shadow-primary/10 border-border/50">
      ...
      <CardContent className="pt-0 space-y-4">
        {/* TODO: Replace with live dining menu/hours API */}
          ...
          <div className="grid grid-cols-1 gap-2">
            {dining.map((d) => (
              <div key={d.name} className="flex items-center justify-between rounded-lg border p-3 bg-card/60">
                ...
                <Badge variant={d.isOpen ? "default" : "secondary"} className={d.isOpen ? "bg-green-600" : "bg-gray-500"}>
          ...
          <div className="grid grid-cols-1 gap-2">
            {otherServices.map((s) => (
              <div key={s.name} className="flex items-center justify-between rounded-lg border p-3 bg-card/60">
                ...
                <Badge variant={s.isOpen ? "default" : "secondary"} className={s.isOpen ? "bg-green-600" : "bg-gray-500"}>
